...
debug = os.environ.get("debug_torchxla") == "1"
...
class DumbReturnHandler:
    ...
    Torch xla does not return such tensors as its group output. But AOTAutograd
    may generate such a graph.
    ...
    def __init__(self, trace_inputs, trace_outputs, trace_inputs_inplace_update_bool):
        ...
        for out_pos, out in enumerate(trace_outputs):
        ...
        if debug:
            print(f"Found {len(self.trace_outputs_pos_to_inputs_pos)} dumb returns: {self.trace_outputs_pos_to_inputs_pos}")
...
def extract_compiled_graph(xla_model: torch.fx.GraphModule, xla_args):
    ...
    if debug:
        print(f"xla_args python ids: {[id(x) for x in xla_args]}")
        print(f"xla_out python ids: {[id(x) for x in xla_out]}")
    ...
    print(f"#args_and_out: {len(args_and_out)}")
    ...
    dumb_return_handler = DumbReturnHandler(xla_args, args_and_out, xla_args_need_update_bool)
    ...
    def optimized_mod(*args):
        ...
        dumb_return_handler.addDumbReturn(args, res)
        if debug:
            print(
